...
After 2 failed attempts to fix the same issue — or if the problem is out of scope — stop trying the same approach. Before showing the options, log the event: run `node scripts/log-event.mjs escalation_prompted flash-debug fail "<brief reason>"` via bash. Then output the following options in your response so the user can choose:
