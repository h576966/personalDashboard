...
import type { SearchMode } from "./useDashboardData";
...
export default function SearchModule({
  ...
  return (
    <div className="border-b border-primary-hover/60 bg-primary-hover dark:border-primary-hover/80">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-2 px-4 py-2 md:grid-cols-[auto_minmax(320px,1fr)_auto_auto_auto] md:items-center">
        <div className="inline-flex h-10 rounded-md border border-white/20 bg-white/5 p-1 text-xs font-medium text-white">
          {(["web", "notes"] as const).map((option) => (
            <button
              key={option}
              type="button"
              ...
              disabled={isLoading}
              className={
                "rounded px-3 transition-colors disabled:cursor-not-allowed disabled:opacity-60 " +
                (mode === option
                  ? "bg-white text-primary-hover"
                  : "text-white hover:bg-white/10")
              }
            >
              {option === "web" ? copy.search.web : copy.search.notes}
            </button>
          ))}
        </div>
        <Input
          ...
          placeholder={isWebMode ? copy.search.webPlaceholder : copy.search.notesPlaceholder}
