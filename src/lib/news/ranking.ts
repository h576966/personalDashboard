...
export interface RankingCluster {
  id: string;
...
export interface RankingPersonalization {
  ...
  savedUrlStatusByUrl: Map<string, "unread" | "read" | "archived">;
  savedHostAffinityByHost: Map<string, number>;
}

interface ScoreClusterInput {
...
function containsBlockedSignal(article: RankingArticle, blockedKeywords: string[]): boolean {
  const text = `${article.title} ${article.description ?? ""} ${article.url}`;
  return containsAny(text, blockedKeywords);
}

export function scoreCluster(
  cluster: RankingCluster,
...
export function rankClusterSelection(
  clusters: RankingCluster[],
  input: ScoreClusterInput,
): RankingCluster[] {
  return clusters
    .map((cluster) => scoreCluster(cluster, input))
