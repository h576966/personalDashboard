...
export function sourceUrl(domain) {
  return `https://${domain}`;
}

export function rowToNewsSource(row) {
...
export function hasUniqueDomains(sources) {
  return new Set(sources.map((source) => source.domain)).size === sources.length;
}

export function hasValidSourceRanges(sources) {
  return sources.every((source) =>
    typeof source.qualityScore === "number" &&
    source.qualityScore >= 0 &&
    source.qualityScore <= 1 &&
